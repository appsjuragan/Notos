...
pub fn menu_bar(
    ...
    egui::menu::bar(ui, |ui| {
        ui.menu_button("File", |ui| {
            if ui.button("New Tab").clicked() { action = Some(MenuAction::NewTab); ui.close_menu(); }
            if ui.button("Open").clicked() { action = Some(MenuAction::Open); ui.close_menu(); }
            if ui.button("Save").clicked() { action = Some(MenuAction::Save); ui.close_menu(); }
            if ui.button("Save As").clicked() { action = Some(MenuAction::SaveAs); ui.close_menu(); }
            ui.separator();
            if ui.button("Exit").clicked() { action = Some(MenuAction::Exit); }
        });

        ui.menu_button("Edit", |ui| {
            if ui.button("Undo").clicked() { action = Some(MenuAction::Undo); ui.close_menu(); }
            if ui.button("Redo").clicked() { action = Some(MenuAction::Redo); ui.close_menu(); }
            ui.separator();
            if ui.button("Find").clicked() { action = Some(MenuAction::Find); ui.close_menu(); }
            if ui.button("Replace").clicked() { action = Some(MenuAction::Replace); ui.close_menu(); }
            if ui.button("Go To...").clicked() { action = Some(MenuAction::GotoLine); ui.close_menu(); }
            ui.separator();
            if ui.button("Time/Date  F5").clicked() { action = Some(MenuAction::TimeDate); ui.close_menu(); }
        });

        ui.menu_button("View", |ui| {
            if ui.checkbox(word_wrap, "Word Wrap").clicked() { action = Some(MenuAction::ToggleWordWrap); ui.close_menu(); }
            if ui.checkbox(show_line_numbers, "Show Line Number").clicked() { action = Some(MenuAction::ToggleLineNumbers); ui.close_menu(); }
            if ui.checkbox(dark_mode, "Dark Mode").clicked() { action = Some(MenuAction::ToggleDarkMode); ui.close_menu(); }
            ui.separator();
            if ui.button("Zoom In").clicked() { action = Some(MenuAction::ZoomIn); }
            if ui.button("Zoom Out").clicked() { action = Some(MenuAction::ZoomOut); }
            if ui.button("Reset Zoom").clicked() { action = Some(MenuAction::ResetZoom); }
            ui.separator();
            ui.menu_button("Change Font", |ui| {
                if ui.selectable_label(editor_font_family == "Monospace", "Monospace").clicked() {
                    action = Some(MenuAction::ChangeFont("Monospace".to_string()));
                    ui.close_menu();
                }
                if ui.selectable_label(editor_font_family == "Proportional", "Proportional").clicked() {
                ...
                if !custom_fonts.is_empty() {
                    ...
                    for name in custom_fonts.keys() {
                        if ui.selectable_label(editor_font_family == name, name).clicked() {
...
pub fn tab_bar(ui: &mut Ui, tabs: &[EditorTab], active_tab_id: Option<uuid::Uuid>) -> Option<TabAction> {
    let action = ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
        ...
        if available_size.x > 0.0 {
            ui.allocate_ui_with_layout(available_size, egui::Layout::left_to_right(egui::Align::Center), |ui| {
...
pub fn status_bar(ui: &mut Ui, tabs: &[EditorTab], active_tab_id: Option<uuid::Uuid>, cursor_pos: (usize, usize), zoom_level: f32) -> Option<StatusBarAction> {
    let mut action = None;
    ui.horizontal(|ui| {
                ...
                if let Some(tab) = tabs.get(index) {
                    ui.menu_button(tab.encoding.name(), |ui| {
                        ...
                        if ui.button("Windows-1252 (ANSI)").clicked() {
                            action = Some(StatusBarAction::SetEncoding(tab.id, encoding_rs::WINDOWS_1252));
                            ui.close_menu();
                        }
                         if ui.button("UTF-16LE").clicked() {
                            action = Some(StatusBarAction::SetEncoding(tab.id, encoding_rs::UTF_16LE));
                            ui.close_menu();
                        }
                        if ui.button("UTF-16BE").clicked() {
                            action = Some(StatusBarAction::SetEncoding(tab.id, encoding_rs::UTF_16BE));
                            ui.close_menu();
                        }
                    });
                    ...
                    ui.menu_button(tab.line_ending.name(), |ui| {
                        if ui.button("Windows (CRLF)").clicked() {
                            action = Some(StatusBarAction::SetLineEnding(tab.id, crate::editor::LineEnding::Crlf));
                            ui.close_menu();
                        }
                        if ui.button("Unix (LF)").clicked() {
                            action = Some(StatusBarAction::SetLineEnding(tab.id, crate::editor::LineEnding::Lf));
                            ui.close_menu();
                        }
                        if ui.button("Mac (CR)").clicked() {
                            action = Some(StatusBarAction::SetLineEnding(tab.id, crate::editor::LineEnding::Cr));
                            ui.close_menu();
                        }
                    });
